...
def employee_create_update_personal_info(request, obj_id=None):
    ...
    create_emp_permission = user_employee.sub_has_perm_number('number_of_employees')
    employees = Employee.objects.filter(parent=user_employee.parent)
        
    employee = Employee.objects.filter(id=obj_id).first()
    form = EmployeeForm(request.POST, instance=employee)
    if form.is_valid():
        if obj_id is None and not create_emp_permission:
            error = f"ليس لديك صلاحية لانشاء اكثر من {employees.count()} موظف، يرجى ترقية الباقة"
