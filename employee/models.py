...
class Employee(BaseModel):
    ...
    parent = models.ForeignKey(ParentOrganization, on_delete=models.CASCADE, null=True)
    ...
    class Meta:
        ...
        unique_together = ("employee_first_name", "employee_last_name", "email", "parent")
        permissions = (
    ...
    def save(self, *args, **kwargs):
        ...
        if request:
            if not self.is_active and self.get_archive_condition() is not False:
                self.is_active = True
                super().save(*args, **kwargs)

            if request.user.is_authenticated:                
                if not self.parent:
                    user_employee = request.user.employee_get;parent_organization = user_employee.parent
                    self.parent = parent_organization
...
class EmployeeWorkInformation(BaseModel):
    """
...
class EmployeeBankDetails(HorillaModel):
    ...
    any_other_code1 = models.CharField(max_length=50, verbose_name=_("Bank Code #1"))
...
class BonusPoint(HorillaModel):
    ...
    def bonus_post_save(sender, instance, **_kwargs):
        ...
        """
        user = instance.employee_user_id
        
        if not BonusPoint.objects.filter(employee_id=instance).exists():
            BonusPoint.objects.create(employee_id=instance, modified_by=user)
