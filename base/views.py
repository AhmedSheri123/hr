...
from base.models import CustomizedGroupModel as Group
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                parent = ParentOrganization.objects.get(id=employee.parent.id)
                parent.setup_end = True
                parent.save()
            except:
                pass
            return render(
...
def create_enable_disable_check_in(request):
    """
    ...
    for company in companies:
        if not AttendanceGeneralSetting.objects.filter(company_id=company, modified_by__employee_get__parent=parent).exists():
            AttendanceGeneralSetting.objects.create(company_id=company).save()
    ...
    try:
        if not EmployeeShiftDay.objects.exists():
            ...
            shifts = [EmployeeShiftDay(day=day[0]) for day in days]
            for shift in shifts:
                shift.save()
...
def login_user(request):
    ...
    if request.method == "POST":
        ...
        create_enable_disable_check_in(request)
        if not employee.parent.setup_end:
            return render(
                request,
                "initialize_database/horilla_company.html",
                {"form": CompanyForm(initial={"hq": True})},
    ...
    if request.method == "POST":
        ...
        if create_rotating_shift:
            if rotating_shift_assign_list:
                for rotating_shift_assign in rotating_shift_assign_list:
...
def history_field_settings(request):
    if request.method == "POST":
        ...
        # البحث عن أول سجل من HistoryTrackingFields
        history_object = HistoryTrackingFields.objects.first()

        # إذا كان السجل موجودًا، نقوم بتحديثه
        if history_object:
            ...
            HistoryTrackingFields.objects.create(
                tracking_fields={"tracking_fields": fields},
                work_info_track=(check == "on")
            )
...
def pagination_settings_view(request):
    # print(DynamicPagination.default_manager.all().delete())
    # print(DynamicPagination.objects.filter(user_id=request.user))
    if DynamicPagination.objects.filter(user_id=request.user).exists():
        pagination = DynamicPagination.objects.filter(user_id=request.user).first()
        pagination_form = DynamicPaginationForm(instance=pagination)
        if request.method == "POST":
...
def excel_holiday_import(file):
    ...
    if valid_holidays:
        for valid_holiday in valid_holidays:
            valid_holiday.save()
