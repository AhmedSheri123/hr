...
from base.horilla_managers import OrganizationManager
from horilla import horilla_middlewares
from horilla.horilla_middlewares import _thread_locals
from horilla.methods import get_horilla_model_class
from horilla.models import HorillaModel, BaseModel
...
class Company(HorillaModel):
    ...
    def clean(self):
        ...
        if request is not None:
            if request.user.is_authenticated:
                ...
                parent = user_employee.parent  # استخراج `parent` من العلاقة
                if Company.objects.filter(
                    company=self.company, 
                    address=self.address, 
                    modified_by__employee_get__parent=parent
                ).exclude(id=self.id).exists():
                    raise ValidationError("هذه الشركة والعنوان موجودان بالفعل.")
    ...
    class Meta:
        ...
        # unique_together = ["company", "address", 'modified_by__employee_get__parent']
        app_label = "base"

        constraints = [
            models.UniqueConstraint(
                fields=["company", "address", "modified_by"],
                name="unique_company_address_modified_by"
            )
        ]
...
class EmployeeShiftDay(BaseModel):
    """
...
class BaserequestFile(BaseModel):
...
class MultipleApprovalManagers(BaseModel):
...
class DynamicPagination(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if request and hasattr(request, "user"):
            existing_entry = DynamicPagination.objects.filter(user_id=request.user).first()
            if existing_entry:
                existing_entry.pagination = self.pagination
                super(DynamicPagination, existing_entry).save(*args, **kwargs)
                return
...
class AnnouncementExpire(BaseModel):
    """
...
class AttendanceAllowedIP(BaseModel):
    """
...
class CustomizedGroupModel(Group):
    created_by = models.ForeignKey(
        ...
        related_name="%(class)s_base_created_by",
    )
    ...
    modified_by = models.ForeignKey(
        ...
        related_name="%(class)s_base_modified_by",
    )
